...
def calculate_exposure(score: torch.FloatTensor, group: torch.BoolTensor, style: str='difference') -> torch.FloatTensor:
    '''
    Calculates a loss based on the exposure metric (Singh & Joachims, 2018).
    Uses log_2 discount and normalizes by group size.

    Parameters
    ----------
    - score: ranking scores, with shape (num_nodes, 1)
    - group: boolean group membership, with shape (num_nodes, 1)
    - style: calculate exposure ratio or exposure difference
    '''
    ...
    #print('score', score[:5])
    #indices = torch.argsort(score, dim=0)
    #print('sorted nodes', sorted_nodes)
    #print('indices', indices)
    #ranking = torch.argsort(indices, dim=0)
    #print('sorted indices', sorted_indices)

    #uq_vals, ranking = torch.unique(score, sorted=True, return_inverse=True)
    #cumsum_counts = counts.cumsum(dim=0)
    #possible_ranks = torch.zeros_like(counts)
    #possible_ranks[1:] = cumsum_counts[:-1]
    #ranking = possible_ranks[inverse_ixs]

    #score = torch.transpose(score, 0, 1)
    # use torchsort because it has a backward function for ranking
    #ranking = torchsort.soft_rank(score, regularization_strength=0.0001) # returned ranks are in 1…size(ranking)
    #print('ranking', ranking)
    #ranking = torch.transpose(ranking, 0, 1)

    # get group exposures
    group_1_ranks = score[group]
    group_0_ranks = score[~group]
    group_1_exp = torch.sum(1/torch.log2(group_1_ranks + 2))/group_1_ranks.size(dim=0)
    group_0_exp = torch.sum(1/torch.log2(group_0_ranks + 2))/group_0_ranks.size(dim=0)
    #print('Minority/Majority exposure:', group_1_exp.detach(), group_0_exp.detach())

    # return exposure ratio or exposure difference
    if style=='difference':
        return group_0_exp - group_1_exp #torch.abs(group_1_exp - group_0_exp)
    elif style=='ratio':
        raise NotImplementedError('Exposure ratio as a loss requires normalization and has not been implemented yet')
    else:
        raise ValueError('Exposure style must either be `difference` or `ratio`')
...
class Trainer(object):
    ...
    def __init__(self, args: ArgsNamespace, random_seed: int, save_name_base: str,
                 adj: Union[sp.spmatrix, None]=None,
        ...
        - ...
        - adj: If provided, use this scipy.sparse adjacency matrix instead of loading data from disk
        - groups: If provided, use this boolean numpy array to label nodes with group membership
        """
        self.args = args
        ...
        if groups is not None:
            self.groups = torch.BoolTensor(groups).to(self.device)
        else:
            self.groups = torch.zeros(self.A.shape[0], dtype=torch.bool, requires_grad=True).to(self.device)
    ...
    def train(self, model_name: str, split: int=0):
        ...
        with tqdm(range(self.args.epochs), unit='epochs') as tqdm_bar:
            for epoch in tqdm_bar:
                ...
                exposure_loss = calculate_exposure(score, self.groups)
                ...
                if (time.time() - last_time > 0.5): # slow down updating the postfix of tqdm
                    tqdm_bar.set_postfix_str(f"train loss: {train_loss.detach().item():.3f}, val loss: {val_loss.detach().item():.3f}, exposure loss: {exposure_loss.detach():.3f}")
                    last_time = time.time()

                ####################
                # Save weights
                ####################
                save_perform = combined_loss.detach().item() #val_loss.detach().item()
